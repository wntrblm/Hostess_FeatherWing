...
static int32_t wtr_usb_send_get_dev_desc_request(struct usb_h_pipe* pipe_0, uint8_t* req_buf, uint8_t* rep_buf, uint8_t len) {
...
static int32_t wtr_usb_send_get_conf_desc_request(struct usb_h_pipe* pipe_0, uint8_t cfg_idx, uint8_t* req_buf, uint8_t* rep_buf, uint16_t len)
{
	struct usb_req *req = (struct usb_req *)(req_buf);
	req->bmRequestType  = USB_REQT_RECIP_DEVICE | USB_REQT_TYPE_STANDARD | USB_REQT_DIR_IN;
	req->bRequest       = USB_REQ_GET_DESC;
